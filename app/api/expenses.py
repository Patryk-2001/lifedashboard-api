...
def add_bill(bill: BillCreate, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
...
def bills_today(db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    return get_bills_by_date(db, date.today())

@router.patch("/{bill_id}", response_model=BillResponse)
def toggle_bill(bill_id: int, completado: bool, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
...
def remove_bill(bill_id: int, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
...
@router.get("/summary")
def bills_summary(db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
